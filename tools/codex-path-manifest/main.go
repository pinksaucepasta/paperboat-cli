...
func main() {
	...
	if err := augmentHooksListNativeCWDAlias(r, absSource, clientRequests, clientResponses); err != nil {
		fatal(err)
	}
	if err := augmentSkillsListNativeCWDAlias(r, absSource, clientRequests); err != nil {
		fatal(err)
	}
	if err := augmentThreadStartNativeWorkspaceRootAlias(r, absSource, clientRequests); err != nil {
		fatal(err)
	}
	if err := augmentNativeLaunchCWDRequestAliases(r, absSource, clientRequests); err != nil {
		fatal(err)
	}
...
func augmentHooksListNativeCWDAlias(r *recorder, source string, requests, responses map[string][]pathPattern) error {
	checks := map[string][]string{
		filepath.Join(source, "codex-rs", "app-server-protocol", "src", "protocol", "v2", "plugin.rs"): {
			`pub struct HooksListParams {`,
			`pub cwds: Vec<PathBuf>`,
			`pub struct HooksListResponse {`,
		},
		filepath.Join(source, "codex-rs", "tui", "src", "hooks_rpc.rs"): {
			`RequestId::String(format!("hooks-list-{}", Uuid::new_v4()))`,
			`params: HooksListParams { cwds: vec![cwd] }`,
			`.find(|entry| entry.cwd.as_path() == cwd)`,
		},
		filepath.Join(source, "codex-rs", "tui", "src", "lib.rs"): {
			`if app_server_target.uses_remote_workspace() || default_environment_is_remote {`,
			`return Ok(None);`,
		},
		filepath.Join(source, "codex-rs", "app-server", "src", "request_processors", "catalog_processor.rs"): {
	...
	for file, markers := range checks {
		...
		for _, marker := range markers {
			if !bytes.Contains(body, []byte(marker)) {
				return fmt.Errorf("pinned hooks/list cwd consumer %s no longer contains %q", file, marker)
			}
		}
	}
	if err := replacePathPatternKind(requests, "hooks/list", "/params/cwds/*", "path", "hooks-list-cwd-alias"); err != nil {
		return err
	}
	return replacePathPatternKind(responses, "hooks/list", "/result/data/*/cwd", "path", "hooks-list-cwd-alias")
}

func augmentSkillsListNativeCWDAlias(r *recorder, source string, requests map[string][]pathPattern) error {
	checks := map[string][]string{
		filepath.Join(source, "codex-rs", "app-server-protocol", "src", "protocol", "v2", "plugin.rs"): {
		...
		filepath.Join(source, "codex-rs", "tui", "src", "app", "background_requests.rs"): {
			`let cwd = self.config.cwd.to_path_buf();`,
			`RequestId::String(format!("startup-skills-list-{}", Uuid::new_v4()))`,
			`cwds: vec![cwd],`,
			`force_reload: true,`,
			`AppEvent::SkillsListLoaded { cwd, result }`,
		},
		filepath.Join(source, "codex-rs", "tui", "src", "app", "event_dispatch.rs"): {
			`if cwds_differ(cwd, self.config.cwd.as_path()) => {}`,
			`AppEvent::SkillsListLoaded { result, .. } => {`,
		},
		filepath.Join(source, "codex-rs", "tui", "src", "app", "thread_routing.rs"): {
			`let cwd = self.chat_widget.config_ref().cwd.clone();`,
			`let errors = errors_for_cwd(&cwd, &response);`,
			`self.chat_widget.handle_skills_list_response(response);`,
		},
		filepath.Join(source, "codex-rs", "tui", "src", "app.rs"): {
			`fn errors_for_cwd(cwd: &Path, response: &SkillsListResponse) -> Vec<SkillErrorInfo> {`,
			`.find(|entry| entry.cwd.as_path() == cwd)`,
		},
		filepath.Join(source, "codex-rs", "tui", "src", "chatwidget", "skills.rs"): {
			`.find(|entry| entry.cwd.as_path() == cwd.as_path())`,
		},
		filepath.Join(source, "codex-rs", "tui", "src", "chatwidget", "session_flow.rs"): {
		...
		filepath.Join(source, "codex-rs", "core", "src", "config", "mod.rs"): {
	...
	for file, markers := range checks {
		...
		for _, marker := range markers {
			if !bytes.Contains(body, []byte(marker)) {
				return fmt.Errorf("pinned skills/list cwd consumer %s no longer contains %q", file, marker)
			}
		}
	}
	return replacePathPatternKind(requests, "skills/list", "/params/cwds/*", "path", "skills-list-cwd-alias")
}

func augmentThreadStartNativeWorkspaceRootAlias(r *recorder, source string, requests map[string][]pathPattern) error {
	checks := map[string][]string{
		filepath.Join(source, "codex-rs", "app-server-protocol", "src", "protocol", "v2", "thread.rs"): {
			`pub struct ThreadStartParams {`,
			`pub runtime_workspace_roots: Option<Vec<AbsolutePathBuf>>`,
		},
		filepath.Join(source, "codex-rs", "tui", "src", "app_server_session.rs"): {
		...
		filepath.Join(source, "codex-rs", "tui", "src", "startup_orchestration.rs"): {
	...
	for file, markers := range checks {
		...
		for _, marker := range markers {
			if !bytes.Contains(body, []byte(marker)) {
				return fmt.Errorf("pinned thread/start workspace-root consumer %s no longer contains %q", file, marker)
			}
		}
	}
	return replacePathPatternKind(requests, "thread/start", "/params/runtimeWorkspaceRoots/*", "absolute", "thread-start-runtime-root-alias")
}

func augmentNativeLaunchCWDRequestAliases(r *recorder, source string, requests map[string][]pathPattern) error {
	checks := map[string][]string{
		filepath.Join(source, "codex-rs", "app-server-protocol", "src", "protocol", "v2", "plugin.rs"): {
		...
		filepath.Join(source, "codex-rs", "app-server-protocol", "src", "protocol", "v2", "command_exec.rs"): {
			`pub struct CommandExecParams {`,
			`pub cwd: Option<PathBuf>`,
			`pub struct CommandExecResponse {`,
		},
		filepath.Join(source, "codex-rs", "tui", "src", "app", "background_requests.rs"): {
			`RequestId::String(format!("plugin-list-{}", Uuid::new_v4()))`,
			`cwds: Some(vec![cwd]),`,
			`force_refetch: false,`,
		},
		filepath.Join(source, "codex-rs", "tui", "src", "app_server_session.rs"): {
		...
		filepath.Join(source, "codex-rs", "tui", "src", "workspace_command.rs"): {
			`RequestId::String(format!("workspace-command-{}", Uuid::new_v4()))`,
			`params: CommandExecParams {`,
			`cwd: command.cwd,`,
			`exit_code: response.exit_code,`,
		},
		filepath.Join(source, "codex-rs", "tui", "src", "branch_summary.rs"): {
	...
	for file, markers := range checks {
		...
		for _, marker := range markers {
			if !bytes.Contains(body, []byte(marker)) {
				return fmt.Errorf("pinned native launch cwd consumer %s no longer contains %q", file, marker)
			}
		}
	}
	for _, target := range []struct {
		...
		{method: "plugin/list", pointer: "/params/cwds/*", kind: "absolute"},
		{method: "thread/resume", pointer: "/params/runtimeWorkspaceRoots/*", kind: "absolute"},
		{method: "thread/fork", pointer: "/params/runtimeWorkspaceRoots/*", kind: "absolute"},
		{method: "command/exec", pointer: "/params/cwd", kind: "path"},
	} {
		if err := replacePathPatternKind(requests, target.method, target.pointer, target.kind, "native-launch-cwd-alias"); err != nil {
			return err
		}
	}
	return nil
...
func replacePathPatternKind(methods map[string][]pathPattern, method, pointer, from, to string) error {
	patterns, exists := methods[method]
	if !exists {
		return fmt.Errorf("pinned protocol no longer declares %s", method)
	}
	found := false
	for index := range patterns {
		if patterns[index].Pointer != pointer {
			continue
		}
		if found || patterns[index].Kind != from {
			return fmt.Errorf("pinned protocol %s path %s is not exactly one %s field", method, pointer, from)
		}
		patterns[index].Kind = to
		found = true
	}
	if !found {
		return fmt.Errorf("pinned protocol %s has no %s %s field", method, from, pointer)
	}
