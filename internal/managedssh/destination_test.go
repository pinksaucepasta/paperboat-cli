...
func TestAliasHostRoundTrip(t *testing.T) {
	host, err := AliasHost("Build-01", "PPRBT.DEV")
	if err != nil || host != "build-01.pprbt.dev" {
		t.Fatalf("AliasHost() = %q, %v", host, err)
	}
	alias, err := ParseAliasHost(host, "pprbt.dev")
...
func TestAliasHostRejectsNonCanonicalDestinations(t *testing.T) {
	for _, host := range []string{"pprbt.dev", "a.b.pprbt.dev", "-bad.pprbt.dev", "bad-.pprbt.dev", "bad.other.dev", "127.0.0.1"} {
		if _, err := ParseAliasHost(host, "pprbt.dev"); !errors.Is(err, ErrSSHAliasInvalid) {
			t.Fatalf("ParseAliasHost(%q) error = %v", host, err)
		}
	}
	for _, alias := range []string{"", "-bad", "bad-", "bad.name", "bad/name"} {
		if _, err := AliasHost(alias, "pprbt.dev"); !errors.Is(err, ErrSSHAliasInvalid) {
...
func TestResolveDestinationFencesPort(t *testing.T) {
	input := DestinationInput{Alias: "build", AliasSuffix: "pprbt.dev", RegisteredPort: 2222, RequestedPort: 2222, RegisteredUser: "deploy", HasRegisteredUser: true}
	got, err := ResolveDestination(input)
	if err != nil || got.Host != "build.pprbt.dev" || got.Port != 2222 || got.User != "deploy" {
