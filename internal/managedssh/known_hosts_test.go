...
func TestFormatKnownHostsUsesStrictAliasAndPortAuthority(t *testing.T) {
	...
	defaultPort, err := FormatKnownHosts("Machine.PPRBT", 22, []HostPublicKey{key})
	if err != nil || !strings.HasPrefix(string(defaultPort), "machine.pprbt ssh-ed25519 ") {
		t.Fatalf("default known hosts=%q error=%v", defaultPort, err)
	}
	nonDefault, err := FormatKnownHosts("machine.pprbt", 2222, []HostPublicKey{key})
	if err != nil || !strings.HasPrefix(string(nonDefault), "[machine.pprbt]:2222 ssh-ed25519 ") {
		t.Fatalf("non-default known hosts=%q error=%v", nonDefault, err)
	}
	key.Fingerprint[0] ^= 0xff
	if _, err := FormatKnownHosts("machine.pprbt", 22, []HostPublicKey{key}); err == nil {
