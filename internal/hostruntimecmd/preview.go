...
func runPreview(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	if args[0] == "list" {
		...
		if json.Unmarshal(data, &records) != nil {
			return errors.New("host runtime returned an invalid preview list")
		}
		if len(records) == 0 {
	...
	if json.Unmarshal(data, &record) != nil || record.LogicalName == "" || record.URL == "" {
		return errors.New("host runtime returned an invalid preview")
	}
	if args[0] == "create" {
...
func callAgentPreview(ctx context.Context, payload map[string]any) (json.RawMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("contact local host runtime: %w", err)
	}
	...
	if err != nil || len(responseBody) > maxPreviewResponseBytes {
		return nil, errors.New("host-runtime preview response is unavailable")
	}
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return nil, fmt.Errorf("host-runtime preview operation failed with status %d", response.StatusCode)
	}
	var envelope struct {
	...
	if decoder.Decode(&envelope) != nil || decoder.Decode(&struct{}{}) != io.EOF || len(envelope.Data) == 0 {
		return nil, errors.New("host runtime returned an invalid preview response")
	}
...
func agentPreviewConfiguration() (string, string, error) {
	...
	tokenFile := os.Getenv("PAPERBOAT_RUNTIME_AGENT_TOKEN_FILE")
	u, err := url.Parse(endpoint)
	if err != nil || u.Scheme != "http" || u.User != nil || u.Path != "/v1/preview-registrations" || u.RawQuery != "" || u.Fragment != "" {
		return "", "", errors.New("local host-runtime preview endpoint is unavailable")
	}
