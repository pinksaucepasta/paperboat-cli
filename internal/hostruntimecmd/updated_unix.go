...
func runUpdated(ctx context.Context, args []string, _ io.Writer, stderr io.Writer) error {
	...
	for _, path := range []string{stateRoot, current, rollback, staged, socket, tokenPath} {
	...
	service, err := updated.New(updated.Config{StateRoot: stateRoot, RuntimeCurrent: current, RuntimeRollback: rollback, RuntimeStaged: staged, Active: active, WorkerUID: uid, WorkerGID: gid, SocketPath: socket, Token: token, RepositoryURL: repository, MachineID: machineID, Health: updated.HTTPHealth{Endpoint: healthURL}})
	if err != nil {
		return err
	}
	if *now {
