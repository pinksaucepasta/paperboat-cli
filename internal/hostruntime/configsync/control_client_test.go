...
func TestControlClientCredentialAndLeaseLifecycle(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Content-Type") != "application/json" || r.Header.Get("X-Paperboat-Machine-Proof") == "" {
			t.Fatalf("missing bounded helper headers: %#v", r.Header)
		}
		switch r.URL.Path {
		case "/v1/config/credentials":
			credentialRequests++
			if r.Header.Get("Authorization") != "Bearer helper-identity" || r.Header.Get("X-Paperboat-Machine-Identity") != "" {
		...
		case "/v1/config/leases/acquire":
			...
			writeTestJSON(t, w, map[string]any{"data": Lease{LeaseID: leaseID, RepositoryID: "repo-1", AssignmentID: "assignment-1", EnvironmentID: "env-1", MachineID: "helper-1", FencingToken: 7, BaseRevision: "head-1", ExpiresAt: now.Add(30 * time.Second)}})
		case "/v1/config/leases/renew":
			assertLeaseAuthorization(t, r)
			writeTestJSON(t, w, map[string]any{"data": Lease{LeaseID: leaseID, RepositoryID: "repo-1", AssignmentID: "assignment-1", EnvironmentID: "env-1", MachineID: "helper-1", FencingToken: 7, BaseRevision: "head-1", ExpiresAt: now.Add(time.Minute)}})
	...
	if err := client.ReportStatus(context.Background(), Status{
		...
		MachineID: "helper-1", InstallationGeneration: 1, WarningRevision: "warning-1", PolicyRevision: "policy-1",
...
func TestControlClientRevalidationRetainsUnexpiredRepositoryAccess(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/config/credentials":
			credentialRequests++
			writeTestJSON(t, w, map[string]any{"data": map[string]any{
				"credential": "credential", "environment_id": "env", "machine_id": "helper",
...
func TestControlClientMapsLeaseFailuresAndInvalidatesAuthorization(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/v1/config/credentials" {
			writeTestJSON(t, w, map[string]any{"data": map[string]any{
				"credential": "config-credential", "environment_id": "env", "machine_id": "helper",
...
func TestControlClientRejectsMismatchedLeaseBindings(t *testing.T) {
	...
	for name, mutate := range map[string]func(*Lease){
		...
		"helper":      func(lease *Lease) { lease.MachineID = "other" },
		...
		t.Run(name, func(t *testing.T) {
			server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.URL.Path == "/v1/config/credentials" {
					writeTestJSON(t, w, map[string]any{"data": map[string]any{
						"credential": "credential", "environment_id": "environment", "machine_id": "helper",
						"assignment_id": "assignment", "warning_revision": "warning", "expires_at": now.Add(5 * time.Minute),
					}})
					return
				}
				lease := Lease{
					...
					EnvironmentID: "environment", MachineID: "helper", FencingToken: 1,
...
func assertLeaseAuthorization(t *testing.T, r *http.Request) {
	t.Helper()
	if r.Header.Get("Authorization") != "Bearer config-credential" || r.Header.Get("X-Paperboat-Machine-Identity") != "helper-identity" {
