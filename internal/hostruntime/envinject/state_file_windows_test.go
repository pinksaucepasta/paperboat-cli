...
func TestSecureStateFileAcceptsAtomicFileProtectedWindowsACLRegardlessOfMode(t *testing.T) {
	...
	want := currentEnvironmentStateDACL(t)
	...
	// Windows reports synthetic POSIX mode bits for NTFS files. The state
	// boundary is the protected DACL, not the 0666 mode passed to Go.
...
func TestSecureStateFileRejectsForeignAndUnprotectedWindowsACLs(t *testing.T) {
	trusted := currentEnvironmentStateDACL(t)
	withoutProtection := trusted[2:] // retain D: while removing the P control bit
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			setEnvironmentStateDACL(t, path, test.descriptor, test.protected)
			info, err := os.Lstat(path)
			if err != nil {
				t.Fatal(err)
			}
...
func currentEnvironmentStateDACL(t *testing.T) string {
	t.Helper()
...
func setEnvironmentStateDACL(t *testing.T, path, sddl string, protected bool) {
	t.Helper()
