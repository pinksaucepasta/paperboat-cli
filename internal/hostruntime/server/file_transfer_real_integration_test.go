...
func TestRealCaddyFRPFileTransferHTTP3AndHTTP2(t *testing.T) {
	...
	handler, err := NewFileTransferHandler(FileTransferHandlerConfig{
		...
		Authorizer: func(token string) (Authorizer, error) {
			...
			return authorizerFunc(func(context.Context, protocol.Frame) (Authorization, error) {
				return Authorization{JournalBinding: "integration-binding", EnvironmentID: "env_integration", MachineID: "machine_host", UserID: "user_integration", ClientID: "cli_integration", SessionID: "ses_integration"}, nil
			}), nil
		},
		AuthorizeCreate: func(_ Authorization, request CreateFileTransferRequest) bool {
			return request.InitiatingUserID == "user_integration" && (request.SourceMachineID == "machine_host" || request.DestinationMachineID == "machine_host")
		},
		ResolveDeliveryClient: func(_ Authorization, request CreateFileTransferRequest) (string, error) {
			if request.DestinationMachineID != "machine_host" {
				return "cli_integration", nil
			}
			return "", nil
		},
	})
	...
	connection, err := dialer.Dial(context.Background(), connector.QUIC, connector.Admission{
		OperationID: "op_real_file_transfer", EnvironmentID: "env_integration", MachineID: "machine_integration", Credential: token,
		Endpoint: connector.EdgeEndpoint{Host: "127.0.0.1", Port: uint16(controlPort), QUICPort: uint16(controlPort)},
		Routes:   []connector.RouteHandoff{{RouteID: "route_file_transfer", Revision: 1, Kind: "runtime_https_wss", PublicHost: "transfer.test", ProxyName: "file-transfer-integration", LocalTarget: connector.RouteTarget{Host: "127.0.0.1", Port: uint16(helperPort)}}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	id := createRealTransfer(t, h3Client, endpoint, "fb_real_h3", "machine_host", data, 3)
	cut := len(data) / 3
	...
	reverseID := createRealTransfer(t, h2Client, endpoint, "fb_real_h2", "machine_client", reverseData, 2)
...
func createRealTransfer(t *testing.T, client *http.Client, endpoint, batchID, destinationMachineID string, data []byte, protocolMajor int) string {
	t.Helper()
	sourceMachineID := "machine_client"
	if destinationMachineID != "machine_host" {
		sourceMachineID = "machine_host"
	}
	payload, _ := json.Marshal(CreateFileTransferRequest{BatchID: batchID, SourceMachineID: sourceMachineID, DestinationMachineID: destinationMachineID, InitiatingUserID: "user_integration", SessionID: "ses_integration", Files: []filetransfer.File{{Basename: "opaque.dat", Size: int64(len(data)), SHA256: transferDigest(data)}}})
