...
func NewFileTransferHandler(config FileTransferHandlerConfig) (*FileTransferHandler, error) {
	...
	if config.Service == nil || config.Journal == nil || config.Authorizer == nil || config.AuthorizeCreate == nil || config.MaxCreateBytes < 1024 || config.MaxCreateBytes > 1<<20 {
...
func (h *FileTransferHandler) serveCollection(writer http.ResponseWriter, request *http.Request, requestID string, authorization Authorization) {
	...
	if err := decoder.Decode(&input); err != nil || input.BatchID == "" || input.SourceMachineID == "" || input.DestinationMachineID == "" || input.InitiatingUserID == "" || authorization.SessionID != "" && input.SessionID != authorization.SessionID || !h.config.AuthorizeCreate(authorization, input) {
		writeHTTPError(writer, requestID, "invalid_request", http.StatusBadRequest, false)
		return
	}
	...
	outcome, replay, err := h.config.Journal.Execute(request.Context(), operationID, canonical, func(ctx context.Context) operation.Outcome {
		transfers, createErr := h.config.Service.Create(ctx, filetransfer.CreateRequest{BatchID: input.BatchID, SourceMachineID: input.SourceMachineID, DestinationMachineID: input.DestinationMachineID, InitiatingUserID: input.InitiatingUserID, SessionID: input.SessionID, DeliveryClientID: clientID, Files: input.Files})
		if createErr != nil {
