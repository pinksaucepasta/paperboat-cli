...
func (c *checker) Check(_ context.Context, path, _ string) error {
	...
	if c.failCurrent && filepath.Base(path) == "pb" {
...
func setup(t *testing.T, artifact []byte, health *checker) (Config, ed25519.PrivateKey, string) {
	...
	root := t.TempDir()
	install := filepath.Join(root, "bin", "pb")
...
func TestRecoveryDistinguishesPreBackupAndPostBackup(t *testing.T) {
	...
	staged := filepath.Join(filepath.Dir(install), ".pb-update-test")
...
func TestRecoveryMatrixPreservesOrRestoresVerifiedArtifact(t *testing.T) {
	...
	for _, test := range []recoveryCase{
		...
		t.Run(name, func(t *testing.T) {
			...
			staged := filepath.Join(filepath.Dir(install), ".pb-update-recovery")
			if err := os.WriteFile(staged, artifact, 0o700); err != nil {
				t.Fatal(err)
			}
			if test.backup {
...
func TestRecoveryRejectsInvalidJournalWithoutDeletingUnexplainedState(t *testing.T) {
	for _, entry := range []journal{
		{State: "staged", Version: "invalid", PreviousVersion: "1.0.0", StagedPath: "/tmp/.pb-update-outside"},
		{State: "staged", Version: "1.1.0", PreviousVersion: "0.9.0", StagedPath: "/tmp/.pb-update-outside"},
		{State: "checking", Version: "1.1.0", PreviousVersion: "1.0.0", StagedPath: "/tmp/.pb-update-outside"},
	} {
