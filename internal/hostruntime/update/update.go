...
func (m *Manager) stage(ctx context.Context, artifact Artifact) (string, error) {
	...
	defer body.Close()
	file, err := os.CreateTemp(filepath.Dir(m.config.InstallPath), ".pb-update-*")
	if err != nil {
		return "", err
	}
	path := file.Name()
...
func safeStagedPath(path, directory string) bool {
	...
	return err == nil && !strings.HasPrefix(relative, ".."+string(filepath.Separator)) && relative != ".." && strings.HasPrefix(filepath.Base(path), ".pb-update-")
}
