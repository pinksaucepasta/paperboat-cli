...
func TestFileTransfersPersistOffsetsPendingRecipientAndReceipt(t *testing.T) {
	...
	now := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)
	transfer := FileTransfer{ID: "ft_1", BatchID: "fb_1", SourceMachineID: "machine_host", DestinationMachineID: "machine_client", InitiatingUserID: "user_1", SessionID: "ses_1", DeliveryClientID: "cli_1", Basename: "empty.bin", Size: 0, SHA256: "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", CreatedAt: now, ExpiresAt: now.Add(10 * time.Minute)}
	if err := state.CreateFileTransfers(context.Background(), []FileTransfer{transfer}); err != nil {
		t.Fatal(err)
	}
	if err := state.CompleteFileTransfer(context.Background(), transfer.ID, "machine_host"); err != nil {
		t.Fatal(err)
	}
...
func TestFileTransferOffsetIsCompareAndSwapAndBounded(t *testing.T) {
	...
	defer state.Close()
	now := time.Now().UTC()
	transfer := FileTransfer{ID: "ft_2", BatchID: "fb_2", SourceMachineID: "machine_client", DestinationMachineID: "machine_host", InitiatingUserID: "user_1", SessionID: "ses_1", Basename: "data.bin", Size: 4, SHA256: "3a6eb0790f39ac87c94f3856b2dd2c5d110e6811602261a9a923d3bb23adc8b7", CreatedAt: now, ExpiresAt: now.Add(7 * 24 * time.Hour)}
	if err := state.CreateFileTransfers(context.Background(), []FileTransfer{transfer}); err != nil {
		t.Fatal(err)
	}
	...
	if err := state.CompleteFileTransfer(context.Background(), transfer.ID, "machine_host"); !errors.Is(err, ErrConflict) {
...
func TestFileTransferBatchCreationIsAtomic(t *testing.T) {
	...
	defer state.Close()
	now := time.Now().UTC()
	base := FileTransfer{BatchID: "fb_3", SourceMachineID: "machine_client", DestinationMachineID: "machine_host", InitiatingUserID: "user_1", SessionID: "ses_1", Basename: "a", Size: 0, SHA256: "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", CreatedAt: now, ExpiresAt: now.Add(time.Hour)}
