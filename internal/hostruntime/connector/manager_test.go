...
func admission(generation uint64, jti string, now time.Time) Admission {
	return Admission{OperationID: "op_admit_0001", JTI: jti, Credential: "test-only-connector-admission-credential", EnvironmentID: "env", MachineID: "helper", ConnectorID: "runtime", Generation: generation, EdgePool: "default", EdgeNodeID: "edge_1", Endpoint: EdgeEndpoint{Host: "edge.test", Port: 7000}, Routes: []RouteHandoff{{RouteID: "route_1", Revision: 1, Kind: "runtime_https_wss", PublicHost: "helper.test", ProxyName: "helper_1", LocalTarget: RouteTarget{Host: "127.0.0.1", Port: 8080}}}, ProtocolVersion: "1.0", ExpiresAt: now.Add(time.Minute)}
}
func manager(t *testing.T, dialer Dialer, now time.Time) *Manager {
	t.Helper()
	m, err := New(Config{EnvironmentID: "env", MachineID: "helper", ConnectorID: "runtime", EdgePool: "default", Dialer: dialer, Clock: fixedClock{now}, DrainTimeout: time.Second})
	if err != nil {
		t.Fatal(err)
	}
	return m
...
func TestSelectedTerminalTransportIsTheOnlyDialedMode(t *testing.T) {
	now := time.Now()
	dialer := &fakeDialer{}
	m, err := New(Config{EnvironmentID: "env", MachineID: "helper", ConnectorID: "runtime", EdgePool: "default", Dialer: dialer, Clock: fixedClock{now}, DrainTimeout: time.Second, Transport: TCPDedicated})
	if err != nil {
		t.Fatal(err)
	}
