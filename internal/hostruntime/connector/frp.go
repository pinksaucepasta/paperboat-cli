...
func NewFRPDialer(config FRPDialerConfig) (*FRPDialer, error) {
	...
	for _, kind := range config.RouteKinds {
		if kind != "runtime_https_wss" && kind != "preview_public_https_wss" || routeKinds[kind] {
...
func admissionMetadata(admission Admission) (string, error) {
	...
	}{admission.OperationID, admission.Credential, admission.EnvironmentID, admission.MachineID, admission.ConnectorID, admission.Generation, admission.EdgePool, admission.EdgeNodeID, admission.Routes})
...
func frpProxyIdentity(admission Admission, route RouteHandoff) frpIdentity {
	stable := admission.EnvironmentID + "\x00" + admission.MachineID + "\x00" + admission.ConnectorID + "\x00" + route.RouteID + "\x00" + route.ProxyName
