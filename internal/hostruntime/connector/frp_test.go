...
func TestFRPDialerSelectsOnlyOwnedRouteKinds(t *testing.T) {
	...
	terminal, err := NewFRPDialer(FRPDialerConfig{RouteKinds: []string{"runtime_https_wss"}})
	if err != nil {
		t.Fatal(err)
	}
	...
	combined, err := NewFRPDialer(FRPDialerConfig{RouteKinds: []string{"runtime_https_wss", "preview_public_https_wss"}})
	if err != nil {
		t.Fatal(err)
	}
	if routes := terminal.selectedRoutes(value.Routes); len(routes) != 1 || routes[0].Kind != "runtime_https_wss" {
...
func TestFRPProxyIdentityIsUniquePerAdmissionAndStablePerRoute(t *testing.T) {
	...
	if firstIdentity.name != "pbp_4324bc234a073d8995ea11fbca503c1d" || firstIdentity.group != "pbg_0fa2e51981d17e0d8d2e0ab2ae9f3764" || firstIdentity.groupKey != "68d4686528478fcf2217f6a409379e8bfec4a1cb688edcb9675b9035d04a44c1" {
		t.Fatalf("identity contract changed: %+v", firstIdentity)
	}
}

func TestConnectorPortMatchesTransport(t *testing.T) {
...
func TestAdmissionMetadataCarriesExactHandoffOnce(t *testing.T) {
	...
	if decoded.OperationID != admission.OperationID || decoded.Credential != admission.Credential || decoded.EnvironmentID != admission.EnvironmentID || decoded.MachineID != admission.MachineID || decoded.Generation != admission.Generation || decoded.EdgeNodeID != admission.EdgeNodeID || len(decoded.Routes) != len(admission.Routes) {
