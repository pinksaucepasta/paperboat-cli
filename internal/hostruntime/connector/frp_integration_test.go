...
func TestPinnedFRPRealServerHTTPWorkConnection(t *testing.T) {
	...
	for index, transport := range []Transport{TCPMux, TCPDedicated, QUIC} {
		t.Run(string(transport), func(t *testing.T) {
			...
			admission := Admission{
				OperationID: "op_admit_first", EnvironmentID: "env_test", MachineID: "helper_test", ConnectorID: "runtime", Credential: token, Endpoint: EdgeEndpoint{Host: "127.0.0.1", Port: uint16(controlPort)},
