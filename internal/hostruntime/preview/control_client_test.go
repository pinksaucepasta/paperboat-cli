...
func TestControlClientSignsRegisterAndAcceptsCanonicalIdentity(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		gotBody = string(body)
		if r.Header.Get("Authorization") != "Bearer preview" || r.Header.Get("X-Paperboat-Machine-Identity") != "identity" || r.Header.Get("X-Paperboat-Machine-Proof") != base64.RawURLEncoding.EncodeToString([]byte("proof")) {
